function Res = SSNICLandICLs(subsidy,GT,Edcostscale,aminfedpctg,SLindic)
% Solves for steady states (SS) of NICL, ICL1 and ICL2
% Gustavo Mellior g.mellior@liverpool.ac.uk

%%%%%%%%% Inputs %%%%%%%%%%
% inpvec(1,1) = Subsidy rate
% inpvec(2,1) = 1 -> GT, 0 -> TS (for SS TSGT used when NICL to TSGT without student debt cancellation)
...
if (aminfedpctg==0)&&(subsidy>0.47) % Generate terminal steady state to study no student loan cancellation for transition NICL to TSGT
    ...
    PBENCH  = TSGT.P;
    SLindic = 1;                                                           % Note we override SLindic and NICL choice above, since TSGT parameters different than  baseline, with student loans, is only for transitions starting from NICL
    NICL    = 1;                                                           % However, as aminfedpctg=0, neither student loan system will have mass where a<0
    if GT==1
        itax1 = TSGT.itax(1);
        itax2 = TSGT.itax(2);
    end
...
else % Compute SS in NICL, ICLs with calibration from paper
...
%%%%%%%% Grids %%%%%%%%
% Student loans (kept coarse for reasonable computational time)
gridd.amin            = -0.7;
gridd.aminfed         = -0.523174541337777*aminfedpctg;
gridd.J               = 10;
J                     = gridd.J;
gridd.amax            = 0; 
gridd.a               = linspace(gridd.amin,gridd.amax,gridd.J); 
...
gridd.income          = [gridd.income gridd.wageL*zstn gridd.wageH*reprate gridd.wageH]; % Students, and unemp/emp with college degree
...
baydelta                  = repmat(gridd.badelta,1,NzNed);
...
% db                        = gridd.bdelta;
% Create mesh: b in 1st dimension, a in 2nd dimension, income type in 3rd dimension, education type in 4th
gridd.bb                  = gridd.b*ones(1,gridd.J); 
gridd.aa                  = ones(gridd.I,1)*gridd.a; 
...
ICL2switchUn = (spdiags(-lnpvec,0,MMu,MMu) +...
               [sparse(M,I*(J-1)),tempnp,sparse(M,M);sparse(M,M),sparse(M,I*(J-1)),tempnp]); % Switch matrix for no college education
lnpvec       = [lnpvec;lnp*ones(I*(J-1),1);zeros(I,1)];
ICL2switchEd = (spdiags(-lnpvec,0,MMe,MMe) +...
               [sparse(M,I*(J-1)),tempnp,sparse(M,2*M);sparse(M,M),sparse(M,I*(J-1)),tempnp,sparse(M,M);...
               sparse(M,2*M),sparse(M,I*(J-1)),tempnp]);                   % Switch matrix for educated                    
Bswitchmaker;
%%
...
% Pre-allocate memory and other numerical parameters
vold              = v;
Vbf               = zeros(I,J,Nz);
Vbb               = zeros(I,J,Nz);
